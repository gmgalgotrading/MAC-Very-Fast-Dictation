...
class Notification:
    ...
    def __init__(self, shutdown_callback=None, file_selected_callback=None, text_file_selected_callback=None, clipboard_process_callback=None):
        ...
        # --- MOTOR DE RELOJES ANIMADOS ---
        self.banner_timer = QTimer()
        self.banner_timer.timeout.connect(self._update_banner_animation)
        self.start_time = 0
        self.current_base_text = ""
    ...
    def open_file_dialog(self):
        file_path, _ = QFileDialog.getOpenFileName(
            None, "Selecciona una grabación médica", "", "Archivos de Audio (*.wav *.mp3 *.m4a)",
    ...
    def open_text_file_dialog(self):
        file_path, _ = QFileDialog.getOpenFileName(
            None, "Selecciona un archivo de texto clínico", "", "Archivos de Texto (*.txt *.md *.rtf)",
            options=QFileDialog.Option.DontUseNativeDialog
        )
    ...
    def _trigger_clipboard(self):
        if self.clipboard_process_callback:
            print("\n📋 Intentando procesar desde el portapapeles...")
    ...
    def _update_banner_animation(self):
        ...
        if self.animation_type == "clock":
            frame = self.clock_frames[self.anim_frame % len(self.clock_frames)]
            animated_text = f"{frame} {time_str} {frame}"
        else:
            frame = self.record_frames[self.anim_frame % len(self.record_frames)]
            animated_text = f"{frame} {time_str} {frame}"
    ...
    def show_structuring(self):
        ...
        self.current_base_text = f"🧠 Estructurando texto con IA ({modo_texto})..."
        self.animation_type = "clock"
        self.start_time = time.time()
        self.anim_frame = 0
