...
import subprocess
import signal
from PySide6.QtCore import QTimer  # <--- NUEVA IMPORTACIÓN PARA SOLUCIONAR EL CRASH
...
def signal_handler(sig, frame):
    ...
    if notification:
        QTimer.singleShot(0, notification.quit)
...
def start_recording():
    ...
    if notification:
        # DELEGAR AL HILO PRINCIPAL
        QTimer.singleShot(0, notification.show)
    is_recording = True
...
def stop_recording():
    ...
    if notification:
        # DELEGAR AL HILO PRINCIPAL
        QTimer.singleShot(0, notification.hide)
    is_recording = False
    if listener_thread:
